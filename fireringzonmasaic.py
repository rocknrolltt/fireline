...
for county in CAcountylist:
	...
	# except OSError:
		# pass
    if not os.path.exists('O:/travellers_testing/CA/%s'%county):
        os.makedirs('O:/travellers_testing/CA/%s'%county)
	os.system('gdalwarp O:/travellers_testing/ArcFireLine/FireRing/FRoldSHIA/CA/%s/%sfroshia.img O:/travellers_testing/CA/%s/%sfroshia.img -t_srs "+proj=longlat +ellps=WGS84"'%(county,county,county,county))
	print "ok"
	with rasterio.drivers():
		with rasterio.open(r"O:/travellers_testing/CA/%s/%sfroshia.img"%(county,county),"r") as src:
			meta = src.meta
			...
			colors=[nonzone,zone_1,zone_2,zone_3,zone_4,zone_5]
			cmap=mpl.colors.ListedColormap([nonzone,zone_1,zone_2,zone_3,zone_4,zone_5])
			bounds_color=[0,5,100,105,200,205,300,305,400,405,500,505]
			#ax.imshow(im)
...
#CA Shapefile##
df=GeoDataFrame.from_file(r"O:\Inferno\F\ArcFireLine\Access\CA\CACounty.shp")
...
zone5_patch=mpatches.Patch(color=zone_5, label='shia zone 5')

plt.legend(handles=[nonzone_patch,zone1_patch,zone2_patch,zone3_patch,zone4_patch,zone5_patch],loc=3)
