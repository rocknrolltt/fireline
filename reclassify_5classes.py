...
for county in ABcountylist:
	...
	raster[(raster==0)]=31
	raster[(raster==1)]=32
	raster[(raster>=2) & (raster<= 3)]=33
	raster[(raster>=4) & (raster<=12)]=34
	raster[(raster>=12) & (raster<=30)]=35
	raster[(raster==255)]=numpy.nan

	driver=gdal.GetDriverByName('gtiff')
	outDs=driver.Create("V:/FireLine/FireRing/Hartford2015/AB/%s/%s_5classes.tiff"%(county,county),raster.shape[1],raster.shape[0],1,gdal.GDT_Int32)
